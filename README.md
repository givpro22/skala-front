...
| 🏆 업적 랭킹 | 업적 해금 순위 TOP 10 + 내 순위 (**회원가입 필요**) |
...
방명록·낙서판을 쓰려고 자동 발급된 **익명 계정은 랭킹에서 제외**합니다.
브라우저를 지우면 사라지는 일회성 신원이라 '순위'로서 의미가 없고,
한 사람이 익명 계정을 여러 개 만들어 목록을 채울 수도 있기 때문입니다.
방명록·좋아요·낙서판은 익명도 그대로 참여할 수 있습니다 — 랭킹만 회원 전용입니다.

> `achievement_leaderboard()` 와 `my_achievement_rank()` 는 **모집단이 반드시 같아야** 합니다.
> 한쪽만 조건이 다르면 목록엔 "3위"인데 아래엔 "내 순위: 4위"로 어긋납니다 (실제로 있었던 버그).

### 히든 업적 · 칭호

**히든 업적**: 이스터에그성 업적(네오·고전 게이머·해커·올빼미·올클리어)은 해금 전까지
목록에 `??? 히든` 으로만 뜨고, 정답 대신 **짧은 힌트**만 보여줍니다. 해금하면 전체 공개됩니다.

**칭호**: 해금한 업적의 이름을 **칭호**로 달 수 있습니다 (업적 패널에서 선택).
선택한 칭호는 **방명록·랭킹·계정 칩**에 닉네임 앞에 표시됩니다.
히든 업적 칭호는 무지개 그라디언트로 **빛나는 특수 효과**가 붙습니다.

> **가짜 칭호는 서버가 막습니다.** 칭호는 `profiles.title` 에 저장되지만,
> 공개 표시 RPC 가 **그 업적을 실제로 보유했는지** 검증해 보유하지 않은 칭호는 내보내지 않습니다.
> (주인 배지와 같은 방식 — 클라이언트를 신뢰하지 않는다)
...
| `achievement_leaderboard()` | 랭킹 TOP 10 (회원만) | 닉네임 + 개수만 |
| `popular_commands()` | 인기 명령어 | 명령어 이름 + 횟수만 (인자 저장 안 함) |

### 주인 사칭 방지

닉네임은 **유니크가 아닙니다.** 방명록에 인사 한 마디 남기려는 사람이
"이미 사용 중인 닉네임입니다"로 막히는 게 더 나쁘기 때문입니다.
대신 두 겹으로 막습니다:

1. **주인 배지** — `profiles.is_owner` 가 true 인 계정에만 `주인` 배지가 붙습니다.
   닉네임이 똑같아도 배지로 구분됩니다. 이 값은 트리거가 지켜서 **사용자가 스스로 켤 수 없고**,
   SQL Editor(=`auth.uid()` 가 null 인 요청)에서만 설정됩니다.
2. **예약어** — `박영서` · `youngseo` · `admin` · `관리자` 등은 주인만 쓸 수 있습니다.
   공백·점·밑줄을 지우고 비교해 `박 영 서` 같은 우회도 걸러냅니다.
   가입 시점에는 **막지 않고 아이디로 대체**합니다 — 여기서 예외를 던지면
   트리거가 롤백되어 계정 생성 자체가 실패합니다.

주인 계정 지정은 SQL Editor 에서 한 번만:
